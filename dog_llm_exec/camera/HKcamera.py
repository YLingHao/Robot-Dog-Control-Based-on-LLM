...
def get_image(cam, data_buf, nPayloadSize):
    ...
    data_buf_res = (c_ubyte * stConvertParam.nDstLen)()
    memmove(byref(data_buf_res), stConvertParam.pDstBuffer, stConvertParam.nDstLen)

    image = np.asarray(data_buf_res)
...
class CameraManager:
    def __init__(self, device_index=0):
        ...
        Enum_device(tlayerType, self.deviceList)
        self.cam, self.data_buf, self.nPayloadSize = enable_device(device_index)
        self.cam.MV_CC_SetFloatValue("ExposureTime", 260000.0)

    def get_frame(self):
        image = get_image(self.cam, self.data_buf, self.nPayloadSize)
