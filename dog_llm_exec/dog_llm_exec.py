...
class DogCommandExecutor:
    ...
    def _exec_moonwalk(self) -> None:
        ...
        try:
            ...
            if not (isinstance(cur, (list, tuple)) and len(cur) >= 3 and cur[0] == 6 and cur[1] == 0 and cur[2] == 0):
                logging.info(f"当前状态 {cur} 不是稳定站立[6,0,0]，等待稳定...")
                start_wait = time.time()
                while time.time() - start_wait < 5.0:
                    time.sleep(0.1)
                    cur = status_listener()
                    if isinstance(cur, (list, tuple)) and len(cur) >= 3 and cur[0] == 6 and cur[1] == 0 and cur[2] == 0:
                        break
                else:
                    logging.warning(f"等待稳定站立超时，当前状态: {cur}，放弃执行太空步")
                    return
            ...
            # 4. sleep(1.5) 后持续发送急停指令 1.5 秒，确保固件响应
            time.sleep(1.5)
            logging.info("太空步主动中止，持续发送急停指令 1.5 秒...")
            stop_deadline = time.time() + 1.5
            while time.time() < stop_deadline:
                self._perform_action(0x21020C0E, 0)
                time.sleep(0.1)
            logging.info("发送收尾动作 0x21010202...")
    ...
    def _exec_motion(self, code: int, param: float, semantic: Optional[str]) -> None:
        ...
        logging.info(f"_exec_motion: code={hex(code)}, param={param}, semantic={semantic}")
